...
$errors = $salonService->validate($input, $_FILES, $salonId);
...
try {
    $salonService->update($salonId, $input, $_FILES, (int) $admin['id']);
    flash_set('success', 'Salon updated.');
} catch (Throwable) {
    old_input_set($input);
    flash_set('error', 'Could not update this salon. Please try again.');
}
